...
const items = [
  ...
  ["specialites","05","Spécialités","Accueillir chaque fonctionnement dans sa singularité",""]
];
export default function Accompagnements() {
  return <StandardPage><PageHero eyebrow="L’approche clinique" title={<>Prendre soin de <em>la psyché.</em></>} lead="Une pratique intégrative, adaptée à votre histoire, à votre rythme et à ce qui vous amène aujourd’hui." />
    <section className="shell section service-list">{items.map(([id,n,t,s,d])=><article id={id} key={id} className={id === "specialites" ? "service-row service-row--specialties" : "service-row"}><span className="service-row__num">{n}</span><div><p className="eyebrow">{t}</p><h2>{s}</h2>{id === "specialites" && <div className="specialty-list"><span>neuroatypies</span><span>Anxiété & surcharge mentale</span><span>Sommeil</span><span>États dépressifs</span><span>Épuisement</span><span>Traumatismes</span><span>Deuils & séparations</span><span>Passages de vie</span><span>Relations & blocages</span><span>Confiance en soi</span><span>Manifestations psychosomatiques</span><span>Addictions · TCA</span></div>}</div>{id !== "specialites" && <p>{d}</p>}</article>)}</section>
    <section className="shell info-band"><p><strong>Cadre des séances</strong><br />Consultations individuelles · Enfants, adolescents et adultes · Fréjus ou visioconférence · Durée et fréquence définies ensemble.</p><p><strong>Important</strong><br />Cet accompagnement ne remplace pas une prise en charge médicale ou un service d’urgence.</p></section><AppointmentCard /></StandardPage>;
}
